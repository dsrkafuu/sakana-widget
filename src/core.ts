...
export class SakanaWidget {
  ...
  constructor(
    options: Omit<Partial<SakanaWidgetOptions>, 'registry'> &
      Pick<SakanaWidgetOptions, 'registry'>,
    character: string
  ) {
  ...
  private run = () => {
    ...
    if (lastRunUnixDiff < 16) {
      _inertia =
        (this.currentCharacter.props.i / this.frameIntervalMs) *
        lastRunUnixDiff;
    }
  ...
  private magicForce = () => {
    ...
    if (Math.random() < 0.1) {
      const available = Object.keys(this.options.registry);
  ...
  setCharacter = (name: string) => {
    const targetChar = this.options.registry[name];
    if (!targetChar) {
   ...
   */
  nextCharacter = () => {
    const _chars = Object.keys(this.options.registry).sort();
